...
/* ------------------------------------------------------------------
   ...
   Concept: "Live status" — instead of styling the page as a filed
   document, the page presents Brian the way a well-run office runs:
   clear, current, and always on schedule. The signature element is
   the status card in the hero (mirroring the "who's on duty" boards
   used in the offices he supports) and the timeline rail through his
   experience, echoing the coordination work itself without literally
   cosplaying as paper.
------------------------------------------------------------------- */
...
export default function Home() {
  ...
  return (
    ...
    >
      ...
      {/* ============ NAV ============ */}
      <nav className="fixed top-0 inset-x-0 z-40 bg-white/90 backdrop-blur border-b border-slate-200">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <button
            ...
            className="sm:hidden text-sm font-medium text-emerald-700"
          >
            Contact
          </button>
        </div>
        <div
          className="h-0.5 bg-emerald-600 transition-all"
      ...
      <header className="max-w-6xl mx-auto px-6 pt-32 sm:pt-40 pb-20 grid lg:grid-cols-[1.15fr_0.85fr] gap-14 items-center">
        <div>
          <p className="font-mono text-xs tracking-widest text-emerald-700 uppercase mb-5">
            Administrative Professional · Nairobi, Kenya
          </p>
          <h1 className="font-display text-5xl sm:text-6xl font-semibold leading-[1.05] text-slate-900">
            Brian Mulama
          </h1>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 border border-slate-300 text-slate-700 px-6 py-3 rounded-full text-sm font-medium hover:border-slate-900 hover:text-slate-900 transition-colors"
            >
              View experience
            </button>
          </div>

          <div className="mt-12 pt-8 border-t border-slate-200 grid grid-cols-3 gap-6 max-w-lg">
            {[
              ["3+", "Years experience"],
              ["3", "Sectors served"],
              ["2", "Languages fluent"],
        ...
        {/* Signature: live status card */}
        <div className="relative bg-white border border-slate-200 rounded-2xl shadow-xl shadow-slate-200/60 p-6">
          <div className="flex items-center gap-4">
            <div className="h-14 w-14 shrink-0 rounded-full bg-emerald-700 text-white flex items-center justify-center font-display text-xl">
              BM
            </div>
            <div>
              <p className="font-semibold text-slate-900">Brian Mulama</p>
              <p className="text-sm text-slate-500">Legal Administrator</p>
            </div>
          </div>
          ...
          <dl className="mt-5 pt-5 border-t border-slate-100 space-y-2 text-sm">
            <div className="flex justify-between">
              <dt className="text-slate-500">Location</dt>
              <dd className="text-slate-800">Nairobi, Kenya</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-slate-500">Languages</dt>
              <dd className="text-slate-800">English, Kiswahili</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-slate-500">Focus areas</dt>
              <dd className="text-slate-800">Legal · Gov · Nonprofit</dd>
            </div>
          </dl>
        </div>
      </header>
      ...
      <section
        id="profile"
        className="max-w-6xl mx-auto px-6 py-20 scroll-mt-20 border-t border-slate-100"
      >
        <div className="grid md:grid-cols-[0.32fr_0.68fr] gap-10">
          <div>
            <p className="font-mono text-xs tracking-widest text-emerald-700 uppercase mb-3">
              Profile
            </p>
      ...
      >
        ...
        <h2 className="font-display text-3xl font-semibold text-slate-900 mb-12">
          Where he's worked
        </h2>
            ...
            <div key={job.role + job.org} className="relative">
              ...
              <div className="flex flex-wrap items-baseline justify-between gap-2">
                <div>
                  ...
                  <p className="text-slate-500 text-sm mt-0.5">{job.org}</p>
                </div>
              ...
              <p className="mt-3 text-slate-600 leading-relaxed">{job.body}</p>
              ...
              <div className="mt-4 bg-emerald-50 border border-emerald-100 rounded-lg px-4 py-3 text-sm text-emerald-900">
      ...
      >
        ...
        <div className="grid sm:grid-cols-2 gap-6">
          {EDUCATION.map((ed) => (
            ...
            >
              <p className="font-mono text-xs text-slate-500">{ed.period}</p>
              <h3 className="font-display text-lg font-semibold text-slate-900 mt-2">
                {ed.title}
              </h3>
              <p className="text-sm text-slate-500 mt-1">{ed.org}</p>
      ...
      >
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {SKILL_GROUPS.map((g) => (
            <div key={g.title}>
              <h4 className="font-mono text-xs uppercase tracking-widest text-slate-400 mb-3">
                {g.title}
              </h4>
      ...
      >
        <div className="bg-slate-900 rounded-3xl px-8 py-16 sm:px-16 text-center">
          <p className="font-mono text-xs tracking-widest text-emerald-400 uppercase mb-4">
            Contact
          </p>
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-4">
            <a
              ...
              className="inline-flex items-center gap-2 bg-white text-slate-900 px-6 py-3 rounded-full text-sm font-medium hover:bg-slate-100 transition-colors"
            >
              <Mail size={16} /> brianmulama052@gmail.com
            </a>
            <a
              href="tel:+254794066425"
              className="inline-flex items-center gap-2 border border-slate-600 text-white px-6 py-3 rounded-full text-sm font-medium hover:border-white transition-colors"
            >
      ...
      <footer className="text-center text-xs text-slate-400 pb-10 font-mono tracking-wide">
